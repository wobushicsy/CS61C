...
mapLoop:
    ...
    slli t4, t0, 2
    # wrong2, didn't times 4
    add t1, t1, t4      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0

    # wrong3, should save counter t0, t1, t2 before call function
    addi sp, sp, -12
    sw t0, 0(sp)
    sw t1, 4(sp)
    sw t2, 8(sp)
    ...
    # wrong4 la a0, 8(s0), la -> lw
    lw a0, 8(s0)        # load the address of the next node into a0
    # wrong5 lw a1, 0(s1), s1 store the address of the function
