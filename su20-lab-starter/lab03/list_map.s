...
map:
    # a0 store the address of the first node
    # a1 store the address of the function
    # this function doesn't need to return a value

    # Prologue: Make space on the stack and back-up registers
    ### YOUR CODE HERE ###
    # since we need to call another function, 
    # then we need to store our returnaddress(ra) and safed register(s0, s1)
    addi sp, sp, -12
    sw ra, 0(sp)
    sw s0, 4(sp)
    sw s1, 8(sp)
    ...
    # THINK: why a0?
    # use a0 is because we will imply this value as a function parameter
    ### YOUR CODE HERE ###
    lw a0, 0(s0)
...
square:
    # use a0 as parameter, calculate a0 * a0, store it in a0 and return
    mul a0 ,a0, a0
    jr ra
